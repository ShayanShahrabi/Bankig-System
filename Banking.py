...
def is_user_valid(user_name, password):
    ...
    if user_name.upper() == "ADMIN":
        ...
        all_lines = accounts_file.readlines()
        found = False
        for line in (all_lines):
            national_code = line[1: 11]
            P = line[71:]
            P = P[0:len(P) - 1]   # remove \n from the end of the `line`
            if(line[0] == '1' and user_name == national_code and password == P):
                found = True
                globalAccountNo = line[46: 56]        # get the account NO in Global AccountNo variable
        accounts_file.close()        
        
        return found
#-----------------------------------------------------------------------------    
# this routin gets the username & password then if it is valid will go to user meny or admin menu
def Login():
    global globalUserName, globalPassword, globalAccountNo
    CreateDataFiles()
    go_on = True
    while go_on:
        ...
        if is_user_valid(globalUserName, globalPassword):
